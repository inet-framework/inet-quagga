...
bgp_capability_mp (struct peer *peer, struct capability_header *hdr)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s OPEN has MP_EXT CAP for afi/safi: %u/%u",
               peer->host__item, mpc.afi, mpc.safi);
...
static int orf_mode_str_max_bgpd = sizeof(orf_mode_str)/sizeof(orf_mode_str[0]);
...
bgp_capability_orf_entry (struct peer *peer, struct capability_header *hdr)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s ORF Cap entry for afi/safi: %u/%u",
	        peer->host__item, entry.mpc.afi, entry.mpc.safi);
  ...
  if (sizeof (struct capability_orf_entry) + (entry.num * 2) > hdr->length)
    {
      zlog_info ("%s ORF Capability entry length error,"
                 " Cap length %u, num %u",
                 peer->host__item, hdr->length, entry.num);
  ...
  for (i = 0 ; i < entry.num ; i++)
    {
      ...
      if (BGP_DEBUG (normal, NORMAL))
        zlog_debug ("%s OPEN has %s ORF capability"
                    ...
                    peer->host__item, LOOKUP (orf_type_str, type),
...
bgp_capability_parse (struct peer *peer, size_t length, u_char **error)
{
  ...
  while (stream_get_getp (s) < end)
	...
	{
	  zlog_info ("%s Capability length error (< header)", peer->host__item);
	  bgp_notify_send (peer, BGP_NOTIFY_CEASE, 0);
	  return -1;
	}
	...
	{
	  zlog_info ("%s Capability length error (< length)", peer->host__item);
	  bgp_notify_send (peer, BGP_NOTIFY_CEASE, 0);
	  return -1;
	}
      ...
      if (stream_get_getp(s) != (start + caphdr.length))
        {
          if (stream_get_getp(s) > (start + caphdr.length))
            zlog_warn ("%s Cap-parser for %s read past cap-length, %u!",
                       peer->host__item, LOOKUP (capcode_str, caphdr.code),
                       caphdr.length);
