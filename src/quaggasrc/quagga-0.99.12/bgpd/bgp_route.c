...
static void
bgp_info_reap (struct_bgp_node *rn, struct bgp_info *ri)
{
  if (ri->next)
...
static void
bgp_info_restore (struct_bgp_node *rn, struct bgp_info *ri)
{
...
static void
bgp_pcount_adjust (struct_bgp_node *rn, struct bgp_info *ri)
{
  assert (rn && rn->table);
 ...
 */
void
bgp_info_set_flag (struct_bgp_node *rn, struct bgp_info *ri, u_int32_t flag)
{
...
void
bgp_info_unset_flag (struct_bgp_node *rn, struct bgp_info *ri, u_int32_t flag)
{
    ...
    if (aspath_count_hops (ri->attr->aspath) >= ri->attr->pathlimit.ttl)
      {
        if (BGP_DEBUG (filter, FILTER))
          zlog_info ("%s [Update:SEND] suppressed, AS-Pathlimit TTL %u exceeded",
                     peer->host__item, ri->attr->pathlimit.ttl);
        return 0;
      }
...
bgp_process_queue_init (void)
{
  bm->process_main_queue
    = work_queue_new (bm->master__item, "process_main_queue");
  bm->process_rsclient_queue
    = work_queue_new (bm->master__item, "process_rsclient_queue");
  ...
  if (peer->pcount[afi][safi] > peer->pmax[afi][safi])
    {
      ...
      zlog (peer->log, LOG_INFO,
	    ...
	    "limit %ld", afi_safi_print (afi, safi), peer->host__item,
      ...
      if (peer->pmax_restart[afi][safi])
	{
	  ...
	  if (BGP_DEBUG (events, EVENTS))
	    zlog_debug ("%s Maximum-prefix restart timer started for %d secs",
			peer->host__item, peer->v_pmax_restart);
  ...
  if (peer->pcount[afi][safi] > (peer->pmax[afi][safi] * peer->pmax_threshold[afi][safi] / 100))
    {
      ...
      zlog (peer->log, LOG_INFO,
	    ...
	    afi_safi_print (afi, safi), peer->host__item, peer->pcount[afi][safi],
	    peer->pmax[afi][safi]);
...
#define CLEAR_QUEUE_NAME_LEN 26 /* "clear 2001:123:123:123::1" */
  ...
  snprintf (wname, CLEAR_QUEUE_NAME_LEN, "clear %s", peer->host__item);
#undef CLEAR_QUEUE_NAME_LEN

  if ( (peer->clear_node_queue = work_queue_new (bm->master__item, wname)) == NULL)
    {
      zlog_err ("%s: Failed to allocate work queue", __func__);
      exit (1);
    }
...
static void
bgp_pathlimit_update_parents (struct bgp *bgp, struct_bgp_node *rn,
                              int ttl_edge)
{
  struct_bgp_node *parent = rn;
...
#define	bgp_network_ttl_cmd	bgp_network_ttl_cmd__VAR
...
#undef	bgp_network_backdoor_ttl_cmd
ALIAS (bgp_network_backdoor,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	bgp_network_backdoor_ttl_cmd	bgp_network_backdoor_ttl_cmd__VAR
...
#undef	bgp_network_mask_ttl_cmd
ALIAS (bgp_network_mask,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	bgp_network_mask_ttl_cmd	bgp_network_mask_ttl_cmd__VAR
...
#undef	bgp_network_mask_backdoor_ttl_cmd
ALIAS (bgp_network_mask_backdoor,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	bgp_network_mask_backdoor_ttl_cmd	bgp_network_mask_backdoor_ttl_cmd__VAR
...
#undef	bgp_network_mask_natural_ttl_cmd
ALIAS (bgp_network_mask_natural,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	bgp_network_mask_natural_ttl_cmd	bgp_network_mask_natural_ttl_cmd__VAR
...
#undef	bgp_network_mask_natural_backdoor_ttl_cmd
ALIAS (bgp_network_mask_natural_backdoor,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	bgp_network_mask_natural_backdoor_ttl_cmd	bgp_network_mask_natural_backdoor_ttl_cmd__VAR
...
ALIAS (no_bgp_network,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	no_bgp_network_ttl_cmd	no_bgp_network_ttl_cmd__VAR
...
#undef	no_bgp_network_backdoor_ttl_cmd
ALIAS (no_bgp_network,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	no_bgp_network_backdoor_ttl_cmd	no_bgp_network_backdoor_ttl_cmd__VAR
...
#undef	no_bgp_network_mask_ttl_cmd
ALIAS (no_bgp_network,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	no_bgp_network_mask_ttl_cmd	no_bgp_network_mask_ttl_cmd__VAR
...
#undef	no_bgp_network_mask_backdoor_ttl_cmd
ALIAS (no_bgp_network_mask,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	no_bgp_network_mask_backdoor_ttl_cmd	no_bgp_network_mask_backdoor_ttl_cmd__VAR
...
#undef	no_bgp_network_mask_natural_ttl_cmd
ALIAS (no_bgp_network_mask_natural,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	no_bgp_network_mask_natural_ttl_cmd	no_bgp_network_mask_natural_ttl_cmd__VAR

#undef	no_bgp_network_mask_natural_backdoor_ttl_cmd
ALIAS (no_bgp_network_mask_natural,
       ...
       "AS-Path hopcount limit attribute\n"
       "AS-Pathlimit TTL, in number of AS-Path hops\n");
#define	no_bgp_network_mask_natural_backdoor_ttl_cmd	no_bgp_network_mask_natural_backdoor_ttl_cmd__VAR
...
bgp_table_stats (struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi)
{
  ...
  thread_execute (bm->master__item, bgp_table_stats_walker, &ts, 0);
...
#define	show_bgp_statistics_cmd	show_bgp_statistics_cmd__VAR

#undef	show_bgp_statistics_vpnv4_cmd
ALIAS (show_bgp_statistics,
       ...
       "Address Family modifier\n"
       "BGP RIB advertisement statistics\n");
#define	show_bgp_statistics_vpnv4_cmd	show_bgp_statistics_vpnv4_cmd__VAR

#undef	show_bgp_statistics_view_cmd
...
#define	show_bgp_statistics_view_cmd	show_bgp_statistics_view_cmd__VAR

#undef	show_bgp_statistics_view_vpnv4_cmd
...
bgp_peer_counts (struct vty *vty, struct peer *peer, afi_t afi, safi_t safi)
{
   ...
   * on just vty_read()).
   */
  thread_execute (bm->master__item, bgp_peer_count_walker, &pcounts, 0);
  
  vty_out (vty, "Prefix counts for %s, %s%s", 
           peer->host__item, afi_safi_print (afi, safi), VTY_NEWLINE);
...
#undef	show_ip_bgp_neighbor_prefix_counts_cmd
...
#define	show_ip_bgp_neighbor_prefix_counts_cmd	show_ip_bgp_neighbor_prefix_counts_cmd__VAR

#undef	show_bgp_ipv6_neighbor_prefix_counts_cmd
...
#define	show_bgp_ipv6_neighbor_prefix_counts_cmd	show_bgp_ipv6_neighbor_prefix_counts_cmd__VAR

#undef	show_ip_bgp_ipv4_neighbor_prefix_counts_cmd
...
#define	show_ip_bgp_ipv4_neighbor_prefix_counts_cmd	show_ip_bgp_ipv4_neighbor_prefix_counts_cmd__VAR

#undef	show_ip_bgp_vpnv4_neighbor_prefix_counts_cmd
...
#define	show_ip_bgp_vpnv4_neighbor_prefix_counts_cmd	show_ip_bgp_vpnv4_neighbor_prefix_counts_cmd__VAR
