...
zserv_flush_data(struct thread *thread)
{
  ...
  switch (buffer_flush_available(client->wb, client->sock))
    ...
    case BUFFER_ERROR:
      ...
      break;
    case BUFFER_PENDING:
      client->t_write = thread_add_write(zebrad.master__item, zserv_flush_data,
      					 client, client->sock);
      break;
    case BUFFER_EMPTY:
      break;
...
zebra_server_send_message(struct zserv *client)
{
  ...
  switch (buffer_write(client->wb, client->sock, STREAM_DATA(client->obuf),
    ...
    case BUFFER_ERROR:
	 ...
	 deleted. */
      client->t_suicide = thread_add_event(zebrad.master__item, zserv_delayed_close,
					   client, 0);
      return -1;
    case BUFFER_EMPTY:
      ...
      break;
    case BUFFER_PENDING:
      THREAD_WRITE_ON(zebrad.master__item, client->t_write,
