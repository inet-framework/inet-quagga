...
irdp_sock_init (void)
{
  ...
  t_irdp_raw = thread_add_read (zebrad.master__item, irdp_read_raw, NULL, sock); 
...
static int
make_advertisement_packet (struct interface_FOO *ifp, 
			   struct prefix *p,
			   struct stream *s)
{
...
static void
irdp_send(struct interface_FOO *ifp, struct prefix *p, struct stream *s)
{
  struct zebra_if *zi=ifp->info;
  struct irdp_interface *irdp=&zi->irdp;
  u_int32_t dst;
...
static void irdp_advertisement (struct interface_FOO *ifp, struct prefix *p)
{
  struct stream *s;
  s = stream_new (128);
