...
static struct list work_queues_lib;
...
work_queue_schedule (struct work_queue *wq, unsigned int delay)
{
  ...
  if ( CHECK_FLAG (wq->flags, WQ_UNPLUGGED)
    ...
    {
      wq->thread = thread_add_background (wq->master__item, work_queue_run, 
                                          wq, delay);
      return 1;
    }
  else
...
#define	show_work_queues_cmd	show_work_queues_cmd__VAR
