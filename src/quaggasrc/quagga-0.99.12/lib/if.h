...
/* Prototypes. */
extern int if_cmp_func (struct interface_FOO *, struct interface_FOO *);
extern struct interface_FOO *if_create (const char *name, int namelen);
extern struct interface_FOO *if_lookup_by_index (unsigned int);
extern struct interface_FOO *if_lookup_exact_address (struct in_addr);
extern struct interface_FOO *if_lookup_address (struct in_addr);

/* These 2 functions are to be used when the ifname argument is terminated
   by a '\0' character: */
extern struct interface_FOO *if_lookup_by_name (const char *ifname);
extern struct interface_FOO *if_get_by_name (const char *ifname);
...
extern struct interface_FOO *if_lookup_by_name_len(const char *ifname,
					       size_t namelen);
extern struct interface_FOO *if_get_by_name_len(const char *ifname, size_t namelen);


/* Delete the interface, but do not free the structure, and leave it in the
   interface list.  It is often advisable to leave the pseudo interface 
   structure because there may be configuration information attached. */
extern void if_delete_retain (struct interface_FOO *);

/* Delete and free the interface structure: calls if_delete_retain and then
   deletes it from the interface list and frees the structure. */
extern void if_delete (struct interface_FOO *);

extern int if_is_up (struct interface_FOO *);
extern int if_is_running (struct interface_FOO *);
extern int if_is_operative (struct interface_FOO *);
extern int if_is_loopback (struct interface_FOO *);
extern int if_is_broadcast (struct interface_FOO *);
extern int if_is_pointopoint (struct interface_FOO *);
extern int if_is_multicast (struct interface_FOO *);
extern void if_add_hook (int, int (*)(struct interface_FOO *));
...
extern void connected_add (struct interface_FOO *, struct connected *);
extern struct connected  *connected_add_by_prefix (struct interface_FOO *,
                                            struct prefix *,
                                            struct prefix *);
extern struct connected  *connected_delete_by_prefix (struct interface_FOO *, 
                                               struct prefix *);
extern struct connected  *connected_lookup_address (struct interface_FOO *, 
                                             struct in_addr);
