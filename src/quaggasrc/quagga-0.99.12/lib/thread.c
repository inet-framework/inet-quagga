...
struct timeval recent_time_lib;
static struct timeval last_recent_time_lib;
/* Relative time, since startup */
static struct timeval relative_time_lib;
static struct timeval relative_time_base_lib;
/* init flag */
static unsigned short timers_inited_lib;

...
thread_cancel (struct thread *thread)
{
  ...
  switch (thread->type)
    ...
    case THREAD_READ:
      ...
      list = &thread->master__item->read;
      break;
    case THREAD_WRITE:
      assert (FD_ISSET (thread->u.fd, &thread->master__item->writefd));
      FD_CLR (thread->u.fd, &thread->master__item->writefd);
      list = &thread->master__item->write;
      break;
    case THREAD_TIMER:
      list = &thread->master__item->timer;
      break;
    case THREAD_EVENT:
      list = &thread->master__item->event;
      break;
    case THREAD_READY:
      list = &thread->master__item->ready;
      break;
    case THREAD_BACKGROUND:
      list = &thread->master__item->background;
      break;
    default:
      return;
      break;
      ...
      if (FD_ISSET (THREAD_FD (thread), fdset))
        {
          ...
          thread_list_delete (list, thread);
          thread_list_add (&thread->master__item->ready, thread);
