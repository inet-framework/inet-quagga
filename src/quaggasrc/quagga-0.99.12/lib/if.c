...
if_create (const char *name, int namelen)
{
  ...
  ifp = XCALLOC (MTYPE_IF, sizeof (struct interface_FOO));
...
void
if_delete_retain (struct interface_FOO *ifp)
{
 ...
 * a seperate struct interface_FOO for each logical interface, so config
