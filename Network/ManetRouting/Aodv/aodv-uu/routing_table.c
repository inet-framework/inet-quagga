				     ...
				     u_int16_t flags, unsigned int ifindex)
{
	...
	/* Add route to omnet inet routing table ... */
        nm.s_addr = IPAddress(next.s_addr).getNetworkMask().getInt();
	omnet_chg_rte(dest_addr, next, nm, hops,false);
#endif
#endif
				     ...
				     u_int16_t flags)
{
	...
	/* Add route to omnet inet routing table ... */
        nm.s_addr = IPAddress(next.s_addr).getNetworkMask().getInt();
	omnet_chg_rte(rt->dest_addr, next, nm, hops,false);
#endif
#endif
...
#ifdef OMNETPP
void NS_CLASS  omnet_chg_rte (struct in_addr dst, struct in_addr gtwy, struct in_addr netm,
		  short int hops,bool del_entry)
{
	/* Add route to kernel routing table ... */
	IPAddress desAddress(dst.s_addr);
	RoutingEntry *entry=NULL;


    	for (int i=inet_rt->numRoutingEntries(); i>0 ; --i)
    	{
        	RoutingEntry *e = inet_rt->routingEntry(i-1);
        	if (desAddress == e->host)
                  {
	            if (!inet_rt->deleteRoutingEntry(e))
	...
	entry = new   RoutingEntry();
	IPAddress netmask(netm.s_addr);
	IPAddress gateway(gtwy.s_addr);

	/// Destination
	entry->host= desAddress;
	/// Route mask
	...
	entry->interfaceName = interface80211ptr->name();
	entry->interfacePtr= interface80211ptr;
	/// Route type: Direct or Remote
	if(entry->gateway.isUnspecified())
		entry->type = RoutingEntry::DIRECT;
	else
		entry->type = RoutingEntry::REMOTE;
	/// Source of route, MANUAL by reading a file,
	/// routing protocol name otherwise
	entry->source=RoutingEntry::MANET;
	inet_rt->addRoutingEntry(entry);
}


bool NS_CLASS  omnet_exist_rte (struct in_addr dst)
{
	/* Add route to kernel routing table ... */
	IPAddress desAddress(dst.s_addr);
	RoutingEntry *e=NULL;
